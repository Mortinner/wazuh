...
install_remote_agent(){
        #sshpass -p "$password" ssh -o StrictHostKeyChecking=accept-new "$user@$ip" curl -sO https://guardar_En_Algun_sitio/deploy_agent.sh && sudo bash ./deploy_agent.sh $ip_server
}
